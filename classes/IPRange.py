...
class IPRange:
    ...
    def __init__(self, *, first_ip=None, second_ip=None, cidr=None):
        ...
        if cidr is not None:
            try:
                ...
                for i in range(IP.octets):
                    ...
                    netmask_octet = ((2**IP.bits_per_octet)-1) ^ ((2**(IP.bits_per_octet-bits))-1)
                ...
                print(e)
                raise ValueError("Incorrect CIDR input to IPRange. Must be a valid instance of type CIDR.")
        ...
        # determine # of hosts (including start/end)
    ...
    def does_overlap(self, other):
        ...
        if self.is_within(other):
            return True
        # external
        return False if self.range[1] < other.range[0] or self.range[0] > other.range[1] else True
